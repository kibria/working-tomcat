...
/** Added code to remove Applet's because they've got the same error as selectboxes in some browsers **/
if(bw.dom&&!bw.op){
  makeCM.prototype.appl=0
  makeCM.prototype.onshow+=";this.hideapplet(pm,pm.subx,pm.suby,maxw,maxh,pm.lev)"
  makeCM.prototype.hideapplet=function(pm,x,y,w,h,l){
    var applx,apply,applw,applh,i
    if(!this.appl){
      this.appl=this.doc.getElementsByTagName("APPLET")
		  this.appl.level=0
    }
    var appl=this.appl
    for(i=0;i<appl.length;i++){
			applx=0; apply=0; var applp;
			if(appl[i].offsetParent){selp=appl[i]; while(selp.offsetParent){selp=selp.offsetParent; applx+=selp.offsetLeft; apply+=selp.offsetTop;}}
			applx+=appl[i].offsetLeft; apply+=appl[i].offsetTop
			applw=appl[i].offsetWidth; applh=appl[i].offsetHeight
			if(applx+applw>x && applx<x+w && apply+applh>y && apply<y+h){
				if(appl[i].style.visibility!="hidden"){appl[i].level=l; appl[i].style.visibility="hidden"; if(pm){ if(!pm.mout) pm.mout=""; pm.mout+=this.name+".appl["+i+"].style.visibility='visible';"}}
      }else if(l<=appl[i].level && !(pm&&l==0)) appl[i].style.visibility="visible"
    }
  }
}
